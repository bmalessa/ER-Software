 ...
 *******************************************************************************/
package de.protin.support.swt.custom;
...
public class StyledText extends Canvas {
	...
	void print() {
		...
		for (int i = startLine; i <= endLine && page <= endPage; i++) {
			...
			//02.08.2020 --- eingefügt, um PageBreak auf den Printer zu erzwingen
			
			if(page <= endPage && paintY > clientArea.y && layout.getText().equals("###PAGEBREAK###")) {
				printer.endPage();
				page++;
				if (page <= endPage) {
					printer.startPage();
...
//			}
		...
		//31.07.2020 --- eingefügt, um auch Image auf den Printer zu bringen
		TextStyle[] styles = layout.getStyles();
		int[] ranges = null;
		int lineOffset = styledText.getContent().getOffsetAtLine(index);
		for (int i = 0; i < styles.length; i++) {
			if (styles[i].metrics != null) {
				...
				styledText.paintObject(gc, x, y, lineAscent, metrics.getDescent(), style, null, 0);
			}
		}
	}
	/**
